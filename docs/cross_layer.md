...
### Patch-location & layer predictors

The retrieval-based patch-location predictor (`memory/predict_location.py`)
and the trained layer-prediction head (`memory/cross_layer/`) build on the
analyzer modes documented above. They live in the private research repo
(**syzfix-research**, `docs/cross_layer.md` there).
